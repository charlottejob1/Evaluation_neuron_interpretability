"""
End-to-end MIG (Mutual Information Gap) studies on simulated data (one PNG per study).
...
Studies (aligned with probability / distance-correlation experiments):
  1) Per-neuron MIG vs beta (0.0 to 1.0, step 0.1) -- blue box plot
...
_SIM_DIR = os.path.join(_REPO, "neuron_simulation")
if _SIM_DIR not in sys.path:
    sys.path.insert(0, _SIM_DIR)

from data_generation import generate_structured_dataset
from mig_metrics_simulation import mig_rows_from_result, run_mig_pipeline
...
DEFAULTS = {
    ...
    "alpha_exp_scale": 1.0,
    "seed": 12345,
    "output_dir": "plots_mig_simulation",
...
METRIC_COLUMN = "per_concept_mig"
METRIC_LABEL = "Per-factor normalized MI gap (paper MIG, Eq. 6)"
...
class RunParameterLog:
    ...
    def _global_block(self) -> str:
        ...
        lines = [
            ...
            f"seed: {a.seed}",
            "",
            "MIG inputs: mean concept activity μ_c (concept_activities), neuron z (Z)",
            "",
...
def _rows_to_dataframe(rows: List[Dict[str, object]]) -> pd.DataFrame:
    ...
    if "mig_dataset" in df.columns:
        df["mig_dataset"] = df["mig_dataset"].astype(float)
...
def _require_plot_columns(df: pd.DataFrame, columns: List[str], study_name: str) -> bool:
    if df.empty:
        print(f"[{study_name}] No MIG rows produced; skipping plot.")
        return False
...
def _mig_ylim(values: np.ndarray) -> tuple:
    if len(values) == 0:
        return (0.0, 1.0)
...
def _run_and_collect_rows(**pipeline_kwargs) -> List[Dict[str, object]]:
    result = run_mig_pipeline(**pipeline_kwargs)
    return mig_rows_from_result(result)
...
def study_beta_sweep(
    ...
    if param_log is not None:
        param_log.add_section(
            "beta_sweep (01_mig_by_beta.png)",
            {
                ...
                "output_csv": "mig_beta_sweep.csv",
            },
        )

    for beta in betas:
        print(f"[beta sweep] beta={beta:.1f}")
        batch = _run_and_collect_rows(
            n_variables=FIXED_M,
            n_concepts=FIXED_K,
            beta=beta,
            overlap_profile=BASE_OVERLAP_PROFILE,
    ...
    df.to_csv(os.path.join(out_dir, "mig_beta_sweep.csv"), index=False)
    ...
    plt.ylim(*_mig_ylim(df[METRIC_COLUMN].values))
    plt.xlabel("beta")
    plt.ylabel(METRIC_LABEL)
    plt.title("Influence of beta on per-factor MIG distribution (paper Eq. 6)")
    plt.grid(True, axis="y", alpha=0.3)
    plt.tight_layout()
    path = os.path.join(out_dir, "01_mig_by_beta.png")
...
def study_M_sweep(
    ...
    df = _rows_to_dataframe(rows)
    df.to_csv(os.path.join(out_dir, "mig_M_sweep.csv"), index=False)
    ...
    plt.ylabel(METRIC_LABEL)
    plt.title("Impact of M on per-factor MIG at fixed beta values")
    plt.legend(title="beta", bbox_to_anchor=(1.02, 1), loc="upper left")
    plt.tight_layout()
    path = os.path.join(out_dir, "02_mig_by_M.png")
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"Saved: {path}")
    return df
...
def study_K_sweep(
    ...
    df = _rows_to_dataframe(rows)
    df.to_csv(os.path.join(out_dir, "mig_K_sweep.csv"), index=False)
    ...
    plt.ylabel(METRIC_LABEL)
    plt.title("Impact of K on per-factor MIG at fixed beta values")
    plt.legend(title="beta", bbox_to_anchor=(1.02, 1), loc="upper left")
    plt.tight_layout()
    path = os.path.join(out_dir, "03_mig_by_K.png")
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"Saved: {path}")
    return df
...
def study_overlap_sweep(
    ...
    if param_log is not None:
        param_log.add_section(
            "overlap_sweep (04_mig_by_overlap.png)",
            {
                "n_examples": args.n_examples,
                "M (fixed)": FIXED_M,
                "K (fixed)": FIXED_K,
                ...
                "output_csv": "mig_overlap_sweep.csv",
            },
        )
    ...
    df = _rows_to_dataframe(rows)
    df.to_csv(os.path.join(out_dir, "mig_overlap_sweep.csv"), index=False)
    ...
    plt.ylabel(METRIC_LABEL)
    plt.title("Impact of overlap regime on per-factor MIG at fixed beta values")
    plt.suptitle(
    ...
    plt.legend(title="beta", bbox_to_anchor=(1.02, 1), loc="upper left")
    plt.tight_layout()
    path = os.path.join(out_dir, "04_mig_by_overlap.png")
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"Saved: {path}")
    return df
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run MIG simulation studies and save plots/CSVs.",
        epilog=(
            "Examples:\n"
            "  python test_mig_simulation.py --run-name mig_v1\n"
            "  python test_mig_simulation.py --quick --run-name mig_quick\n"
        ),
